...
function VocabThemeDetail() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: exData } = await supabase.from('vocab_theme_exercises').select('*').eq('theme_id', themeData.id).order('position')
      setTheme(themeData)
      setExercises(exData || [])
      setLoading(false)
    }
  ...
  const goToNext = async () => {
    if (currentIndex + 1 >= exercises.length) {
      ...
      try {
        ...
        await supabase.from('user_vocab_theme_practice').upsert({ user_id: user.id, theme_id: theme.id })
    ...
    return (
      <AppLayout>
        <div className="page">
          ...
          <p className="dashboard-goal">{theme.words.length} mots à découvrir, plusieurs niveaux mélangés.</p>
          ...
          {exercises.length > 0 && (
            <button className="btn-primary" onClick={() => setMode('practice')}>✏️ S'entraîner sur ce thème</button>
          )}
        </div>
      </AppLayout>
    )
      ...
      <AppLayout>
        <div className="page">
          ...
          <div className="lesson-summary">
            ...
            <button className="btn-secondary" style={{ marginTop: '0.5rem' }} onClick={() => navigate('/vocab-themes')}>Autres thèmes</button>
          </div>
        </div>
      </AppLayout>
    )

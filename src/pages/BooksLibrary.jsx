...
function BooksLibrary() {
  ...
  // La lecture "en cours" : progression existante, non terminée, la plus avancée
  const inProgress = books
    .map((b) => ({ book: b, progress: progressMap[b.id] }))
    .filter((x) => x.progress && !x.progress.completed)
    .sort((a, b) => (b.progress.current_page || 0) - (a.progress.current_page || 0))[0]
  ...
  return (
    <AppLayout>
      <div className="page b2-page">
        ...
        {inProgress && (
          <div className="b2-current-card" onClick={() => navigate(inProgress.book.is_series ? `/books/series/${inProgress.book.id}` : `/books/${inProgress.book.id}`)}>
            ...
            <div style={{ padding: '17px' }}>
              ...
              <p className="b2-current-theme">{inProgress.book.theme}</p>

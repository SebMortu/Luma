...
// ---------- Liste ----------
function ListMode({ verbs, groupLabel, masteryMap, onSwitchMode }) {
  return (
    <div>
      ...
      <p className="v2-section-label" style={{ marginTop: '18px' }}>Continuer avec ce groupe</p>
      <div className="v2-switch-row">
        <button className="v2-switch-btn" onClick={() => onSwitchMode('flashcards')}>🃏 Flashcards</button>
        <button className="v2-switch-btn" onClick={() => onSwitchMode('quiz')}>❓ Quiz</button>
        <button className="v2-switch-btn" onClick={() => onSwitchMode('sprint')}>⚡ Défi 60s</button>
      </div>
    </div>
  )
}
...
function Verbs() {
  ...
  const alphaGroups = useMemo(() => {
    const sorted = [...verbs].sort((a, b) => a.base_form.localeCompare(b.base_form))
    const third = Math.ceil(sorted.length / 3)
    const chunks = [sorted.slice(0, third), sorted.slice(third, third * 2), sorted.slice(third * 2)]
    return chunks.filter((c) => c.length > 0).map((c) => ({
      key: `alpha-${c[0].base_form}`,
      label: `${c[0].base_form[0].toUpperCase()} – ${c[c.length - 1].base_form[0].toUpperCase()}`,
      verbIds: new Set(c.map((v) => v.id)),
    }))
  }, [verbs])
  const priorityCount = verbs.filter((v) => v.is_priority).length
  const activeAlphaGroup = alphaGroups.find((g) => g.key === familyFilter)
  const filtered = familyFilter === 'priority' ? verbs.filter((v) => v.is_priority)
    : familyFilter === 'all' ? verbs
    : activeAlphaGroup ? verbs.filter((v) => activeAlphaGroup.verbIds.has(v.id))
    : verbs.filter((v) => v.family === familyFilter)
  const groupLabel = familyFilter === 'priority' ? 'Top 30 prioritaires' : familyFilter === 'all' ? 'Tous les verbes' : activeAlphaGroup ? `Groupe ${activeAlphaGroup.label}` : `Famille : ${familyFilter}`
    ...
    <div className="page v2-page">
      {!mode && (
        <>
          ...
          <div className="sc2-section-head" style={{ marginTop: '14px' }}>
            <span style={{ color: 'var(--text-secondary)' }}>Ou par ordre alphabétique</span>
            <span className="sc2-section-line" />
          </div>
          <div className="v2-group-grid">
            {alphaGroups.map((g) => {
              const groupVerbs = verbs.filter((v) => g.verbIds.has(v.id))
              const groupMastered = groupVerbs.filter((v) => masteryMap.has(v.id)).length
              return (
                <div key={g.key} className={`v2-group-tile ${familyFilter === g.key ? 'selected' : ''}`} onClick={() => setFamilyFilter(g.key)}>
                  <div className="v2-group-top">
                    <span style={{ fontSize: '17px' }}>🔡</span>
                    <span className="v2-group-count">{groupMastered}/{groupVerbs.length}</span>
                  </div>
                  <div className="v2-group-name">{g.label}</div>
                </div>
              )
            })}
          </div>
      ...
      {mode && (
        <>
          ...
          {mode === 'list' && <ListMode verbs={filtered} groupLabel={groupLabel} masteryMap={masteryMap} onSwitchMode={setMode} />}

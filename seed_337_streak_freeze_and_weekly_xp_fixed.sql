-- ============================================
-- LUMA — Streak freeze + XP hebdomadaire (version consolidée et corrigée)
-- Remplace seed_334 et seed_336 : la structure de retour de la fonction change
-- (nouvelles colonnes), il faut donc la SUPPRIMER avant de la recréer —
-- "CREATE OR REPLACE" ne peut pas changer les colonnes de retour d'une fonction.
-- ============================================

-- 1) Colonnes nécessaires (sûres à relancer, IF NOT EXISTS)
alter table user_settings add column if not exists streak_freezes integer not null default 0;
alter table user_settings add column if not exists last_freeze_milestone integer not null default 0;
...
-- 2) Supprime l'ancienne version de la fonction avant de la recréer
drop function if exists award_progress(uuid, integer, integer);

-- 3) Nouvelle fonction complète (streak freeze + XP hebdomadaire)
create function award_progress(
  p_user_id uuid,
  p_xp_gained integer,
